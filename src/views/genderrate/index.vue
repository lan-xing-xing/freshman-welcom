...
export default {
  ...
  mounted() {
    ...
    let totalNum = 100// 指定图形界限的值,即总人数
    let maleNum
    let femaleNum
    let malePay
    let femalePay
    const labelSetting = {
      normal: {
        ...
        formatter: function(param) {
          return (param.value / totalNum * 100).toFixed(2) + '%'
        },
        textStyle: {
          fontSize: 18,
          fontFamily: 'Arial',
    ...
    function myRealTimeDataAndShow() {
      getList('/getRealTimeData' + path).then(response => {
        ...
        maleNum = realTimeData.maleNum
        femaleNum = realTimeData.femaleNum
        malePay = realTimeData.malePay
        femalePay = realTimeData.femalePay
        myCharts.setOption({
          tooltip: {
